...
export function ArgumentsDirective(): QueryValidationRule {
  return {
    create(context) {
      return {
        FragmentSpread(node) {
        ...
        Directive(node, key, parent, path, ancestors) {
          ...
          if (
            !checkDirectivePresentOnFragment(
              fragmentNode,
              'argumentDefinitions',
            )
          ) {
            context.reportError(
              new GraphQLError(
                `Directive 'arguments' used on fragment '${fragmentName}' which is not expecting arguments (@argumentDefinitions directive not present on fragment).`,
                [node],
              ),
            );
          }
