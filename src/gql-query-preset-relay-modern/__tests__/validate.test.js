...
describe('ArgumentsDirective', () => {
  ...
  it("should ignore if fragmentSpread's fragment not found", async () => {
    const errors = await validateSource(
      `
        graphql\`
          fragment Test on User {
  ...
  it.only('should report Wrong type argument value passed inside directive', async () => {
    const errors = await validateSource(
      `
        graphql\`
          query User($boolVar: Boolean!) {
            viewer {
              me {
                ...ProfileUser @arguments(size: $boolVar)
              }
            }
          }
        \`
