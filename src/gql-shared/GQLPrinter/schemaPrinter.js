/* eslint-disable */
// FIXME: remove this file when graphql-js exposes individual print methods
/**
...
export function printFields(options, type) {
  const fieldMap = type.getFields();
  const fields = Object.keys(fieldMap).map(fieldName => fieldMap[fieldName]);
  return fields
    .map(
...
export function printArgs(options, args, indentation = '') {
  if (args.length === 0) {
    return '';
  }
