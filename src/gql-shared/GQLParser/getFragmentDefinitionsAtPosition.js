/* @flow */
import { Source, getLocation, type FragmentDefinitionNode } from 'graphql';
import { toOffset, runParser, BREAK } from 'gql-shared/GQLParser';
import { type GQLPosition, type IParser } from 'gql-shared/types';
import { GQLFragment } from 'gql-shared/GQLFragmentsManager';
import {
  checkFragmentScopesDocument,
  type FragmentScope,
} from 'gql-shared/FragmentScope';

type IContext = {
  getFragments(name: string): Array<GQLFragment>,
  getAllFragments(): Array<GQLFragment>,
  getConfig(): { fragmentScopes: Array<FragmentScope> },
};
...
function extractFragmentsFromSource(
  ...
  runParser(parser, source.body, (stream, state, style) => {
    // console.log({
